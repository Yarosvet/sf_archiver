...
def compress_huffman(
    ...
    """
    __compress(if_stream, of_stream, filename, 'huffman', enable_checksum=enable_checksum)
...
def compress_shannon_fano(
    ...
    As for now, it stores only filename in metadata.
    """
    __compress(if_stream, of_stream, filename, 'fano', enable_checksum=enable_checksum)
